...
func (g *GQueue) Stop() {
	...
	// 单次循环完成：唤醒 -> 等 runDequeue 退出 -> 关闭 jobs
	for _, w := range workers {
		w.dq.Close()
		w.dqWG.Wait()
		w.cond.Broadcast() // 唤醒可能阻塞在 Wait 的出队循环
		w.dequeueWG.Wait() // 确保 runDequeue 已退出（不会再 jobs <- fn）
		close(w.jobs)      // 关闭执行池，worker 协程自然退出
	}
}
...
func (q *GQueueArray) String() string { q.RLock(); str := q.queue.String(); q.RUnlock(); return str }
...
type gqWorker struct {
	...
	gq        *GQueue
	cond      *sync.Cond
	inflight  sync.WaitGroup // 正在执行中的 job 数
	dequeueWG sync.WaitGroup // 等待出队协程结束
	dqWG      sync.WaitGroup // 等待延迟队列消费协程结束
}
...
func (w *gqWorker) startDelayConsumer() {
	// 消费延迟队列到期任务：到期后直接 run（不再经过 ready 队列，降低延迟）
	w.dqWG.Add(1)
	go func() {
		defer w.dqWG.Done()
		for {
...
func newGQWorker(limit int, maxRetry int, logger util.ItfLogger, gq *GQueue) *gqWorker {
	...
	// 启动 limit 个固定 worker 协程消费 jobs
	for i := 0; i < limit; i++ {
		go workerFunc(jobs, logger)
	}
	return &gqWorker{
		runner:   func(fn func()) { jobs <- fn }, // 阻塞投递：实现背压
		jobs:     jobs,
