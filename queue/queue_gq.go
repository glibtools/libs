...
func LoadAntsPool(sizes ...int) *ants.Pool {
	sizeOfPool := 100
	if len(sizes) > 0 && sizes[0] > 0 {
		sizeOfPool = sizes[0]
	}
	return util.LoadSingleInstance(
		fmt.Sprintf("ants_pool_%d", sizeOfPool),
		func() *ants.Pool {
			pool, _ := ants.NewPool(sizeOfPool)
			return pool
		},
	)
}
...
func NewGQueue(opt GQueueOption) *GQueue {
	...
	if opt.MaxConcurrent < 10 {
		opt.MaxConcurrent = 10
	}
	if opt.MaxRetry < 1 {
...
func concurrencyLimitRunner(limit int) func(fn func()) {
	if limit <= 1 {
		return func(fn func()) {
			fn()
		}
...
func newGQWorker(
	...
	runner := concurrencyLimitRunner(limit)
	w := &gqWorker{
		runner:   runner,
