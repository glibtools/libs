...
func (c *CurdParams) Create(args ...any) (err error) {
	...
	// create
	return c.prepareDB(args...).Transaction(func(tx *gorm.DB) error {
...
func (c *CurdParams) Update(args ...any) (err error) {
	...
	if err = c.Values.ToBean(newBean); err != nil {
		return
	}
	if c.BeforeCall != nil {
	...
	delete(mps, "id")
	return c.prepareDB(args...).Transaction(func(tx *gorm.DB) error {
		return dbWithWhere(c.wrapper(tx), c.Where).Model(oldBeanData).Updates(mps).Error
	})
}
