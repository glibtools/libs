...
func (c *CurdParams) Update(args ...any) (err error) {
	...
	if err = dbWithWhere(db, c.Where).Where("id = ?", idUint64).Take(oldBeanData).Error; err != nil {
		return j2rpc.NewError(400, err.Error())
	}
	...
	// merge old + user changes into updateBeanData
	t := util.Bean2Map(oldBeanData)
	for k, v := range mps {
		t[k] = v
	}
	updateBeanData := util.NewValue(bean)
	if err = t.ToBean(updateBeanData); err != nil {
		return
	}

	//BeforeCall runs on the fully-merged bean (consistent with Create).
	//Detect any extra columns it sets and append them to selectColumns.
	if c.BeforeCall != nil {
		preCBMap := util.Bean2Map(updateBeanData)
		c.BeforeCall(updateBeanData)
		postCBMap := util.Bean2Map(updateBeanData)
		for _, col := range columns {
			if col == "id" || slices.Contains(selectColumns, col) {
				continue
			}
			if !reflect.DeepEqual(preCBMap[col], postCBMap[col]) {
				selectColumns = append(selectColumns, col)
			}
		}
	}

	//CheckBeforeUpdate receives the fully-merged updateBeanData (not a partial bean)
	if c.CheckBeforeUpdate != nil {
		if err = c.CheckBeforeUpdate(oldBeanData, updateBeanData); err != nil {
			return
		}
	}

	//if bean has field `Version` and it's not in selectColumns, add it to selectColumns for optimistic locking
	if util.BeanHasFieldCallback(oldBeanData, versionField) && !slices.Contains(selectColumns, versionField) {
		selectColumns = append(selectColumns, versionField)
	}

	err = db.Transaction(func(tx *gorm.DB) error {
		return dbWithWhere(c.wrapper(tx), c.Where).
			Select(selectColumns).
			Omit("id").
			Clauses(clause.Returning{}).
			Updates(updateBeanData).Error
	})
	if err != nil {
		return
	}
	if c.AfterCall != nil {
		return c.AfterCall(updateBeanData)
	}
	return
}
