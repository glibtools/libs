...
func (p *gormPluginCache) query(db *gorm.DB) {
	...
	_, err, _ := localSingleFlight.Do(sfKey, func() (interface{}, error) {
		...
		qdb.Statement.Dest = reflect.New(reflect.TypeOf(db.Statement.Dest).Elem()).Interface()
		qdb.Raw(qdb.Statement.SQL.String(), qdb.Statement.Vars...).Scan(qdb.Statement.Dest)
...
func searchCacheKey(pre, table, sq string) string {
	// fmt.Sprintf("%s%s[%s]", searchKeyPrefix(pre, table), separator, sq)
