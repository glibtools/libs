...
func (p *gormPluginCache) getCache(stm *gorm.Statement) (hit bool) {
	...
	if !isPrimary {
		var pKey string
		pKey, hit = p.queryCacheSearch(stm, key)
		if hit {
...
func (p *gormPluginCache) query(db *gorm.DB) {
	...
	//color.Cyan("singleflight key: %s", sfKey)
	data, err, _ := localSingleFlight.Do(sfKey, func() (interface{}, error) {
		var err error
		p.queryWithCache(db)
		if db.Error != nil {
			return nil, db.Error
		}
		c := gm2call{RowsAffected: db.RowsAffected}
		if c.RowsAffected > 0 && db.Statement.Dest != nil {
			c.Data, err = util.Marshal(db.Statement.Dest)
		}
		return c, err
	})
	db.Error = err
	...
	if len(c.Data) > 0 {
		err = util.Unmarshal(c.Data, db.Statement.Dest)
		if err != nil {
			_ = db.AddError(err)
			return
		}
...
func clauseFindPrimaryCacheKey(expr clause.Expression, primaryDBName string) (str string) {
	...
	case clause.Expr:
		...
		if v, ok := reRegexCache.Load(primaryDBName); ok {
			...
			r := regexp.MustCompile("^" + regexp.QuoteMeta(primaryDBName) + `=\d+$`)
...
func singleflightKey(p *gormPluginCache, stm *gorm.Statement) string {
	...
	if stm != nil {
		if stm.Dest != nil {
			destTyp = reflect.TypeOf(stm.Dest).String()
		}
		table = stm.Table
	}
	return fmt.Sprintf("%s:%s:%s:%s:%s", globalPrefix, p.Prefix, table, destTyp, sq)
}
...
func sqlKeyFromStmt(stm *gorm.Statement) string {
	...
	// 生成包含实参的稳定 SQL 字符串
	return stm.DB.Dialector.Explain(stm.SQL.String(), stm.Vars...)
}
