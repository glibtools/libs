...
func (p *gormPluginCache) afterDelete(db *gorm.DB) {
	...
	//color.Red("line 136 DropPrefix; key:%s", searchKey1)
	...
	if stm.Schema == nil || stm.Schema.ModelType.Kind() != reflect.Struct {
		...
		//color.Red("line 140 DropPrefix; key:%s", primaryKey1)
		return
	}
	if stm.ReflectValue.Kind() != reflect.Struct {
		p.DropPrefix(primaryKey1)
		//color.Red("line 145 DropPrefix; key:%s", primaryKey1)
		return
	}
	pk := p.findPrimaryCacheKey(stm, true)
	if pk != "" {
		p.Del(pk)
		//color.Red("line 151 delete primary cache key: %s", pk)
		return
	}
	...
	if idStr != "" {
		...
		p.Del(_key)
		//color.Red("line 158 delete primary cache key: %s", _key)
		return
	}
	_key := primaryKeyPrefix(p.Prefix, stm.Table)
	p.DropPrefix(_key)
	//color.Red("line 163 DropPrefix; key:%s", _key)
}
