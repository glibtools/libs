...
func (p *JSONTime) FromDB(data []byte) error {
	rt, err := parseInlocation2jsonTime(string(data))
	if err != nil {
		return err
	}
	*p = *rt
...
func (p *JSONTime) UnmarshalJSON(data []byte) error {
	...
	jt, err := parseInlocation2jsonTime(str)
	if err != nil {
...
func ToDatetime(in string) (JSONTime, error) {
	rt, err := parseInlocation2jsonTime(in)
	if err != nil {
		return JSONTime(time.Time{}), err
	}
	return *rt, nil
}
...
func parseInlocation2jsonTime(str string) (rt *JSONTime, err error) {
	formats := []string{
	...
	err = fmt.Errorf("parseInlocation2jsonTime: unsupported time format: %s", str)
	return
}
